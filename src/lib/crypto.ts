...
export function getEncryptedText<T>(input: T): T | string {
	...
	if (config.essentials.applyEncryption && config.essentials.secretCode) {
		// Convert input to JSON string if it's not already a string
		const output =
			typeof input === 'string' ? input : JSON.stringify(input);
		return encrypt(output, config.essentials.secretCode);
	}

	return input;
}

export function getDecryptedText<T>(input: T): T | string {
	if (config.essentials.applyEncryption && config.essentials.secretCode) {
		const decrypted =
			typeof input === 'string'
				? decrypt(input, config.essentials.secretCode)
				: input;
