...
export const validCreateGBPAuditReportBody = catchAsync(async (req, res, next) => {
	try {
		const { location_id, scheduling, keywords, user } = req.body;

		if (!location_id || !scheduling || !keywords) {
			return responseWrapper(
				res,
				'',
				'Please Provide Required Fields: location_id, scheduling, keywords',
...
export const validFetchGBPAuditReportBody = catchAsync(async (req, res, next) => {
	try {
